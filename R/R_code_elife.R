#######################################################################
#
# Copyright (C) 2023 CNRS and University of Strasbourg
#
...
# all quantitative variables were standardized (using z-scores)
# in multi-factor models

model_1 <- read.csv("model_1.csv", header = TRUE, stringsAsFactors = FALSE)
model_1$zbody_mass_male = scale(model_1$body_mass_male, center = TRUE, scale = TRUE)
model_1$zbody_mass_change_during_mating = scale(model_1$body_mass_change_during_mating,
...
model_2 <- read.csv("model_2.csv", header = TRUE, stringsAsFactors = FALSE)
...
model_3 <- read.csv("model_3.csv", header = TRUE, stringsAsFactors = FALSE)
...
comb_model_1 <- comparative.data(phy = Tree_m_1_comp, data = model_1, names.col = species,
...
comb_model_2 <- comparative.data(phy = Tree_m_2_comp, data = model_2, names.col = species,
                                 vcv = TRUE, na.omit=FALSE, warn.dropped=TRUE)
str(comb_model_2)
...
comb_model_3 <- comparative.data(phy = Tree_m_3_comp,data = model_3, names.col = species,
                                 vcv = TRUE, na.omit = FALSE, warn.dropped = TRUE)
str(comb_model_3)
...
# PGLS models
#
# pgls model with lambda evaluation by ML (1=high phylogenetic
# covariance; 0 = no phylogenetic covariance)

#
# Model 1
#
fit1_1 <- pgls(protandry~late_mating + foodstoring + body_mass_male +
               dimorphism_body_mass_emergence + body_mass_change_during_mating * min_temper +
...
fit1_2 <- pgls(protandry~zbody_mass_male + zbody_mass_change_during_mating + zmin_temper +
               zlate_mating, data = comb_model_1, lambda = "ML")
summary(fit1_2)
...
# 2) Homoscedasticity
#
# Ploting residual value vs fitted value allows to check the
# homoscedasticity. The residuals have to be randomly distributed
# around the 0 line, approximately form a horizontal band around the
# 0 line with no extreme value from the basic random pattern of
# residuals. 
...
fit1_3 <- pgls(protandry~body_mass_male + body_mass_change_during_mating + min_temper + late_mating,
               data = comb_model_1, lambda = "ML")
summary(fit1_3)

eq1 = function(x){
    (20.6398273 + (0.0037926*499.9) + (0.712107*-8.31) + (-1.9893747*1.55) + (-0.4381697*x))
}

ggplot(model_1, aes(x = body_mass_change_during_mating, y = protandry, colour = min_temper)) +
    geom_point(size = 4) + theme_classic() +
    labs(x = "Body mass change during mating (% body mass)", y = "Protandry (day)") +
    scale_y_continuous(limits = c(0,55), expand = c(0, 0)) +
    scale_color_gradient2(
        "Min temperature (\u00b0C)", midpoint = -8.31, low = "blue4", mid = "snow3",
        high = "red4", space = "Lab"
...
# Model 2
#
fit2_1 <- pgls(sex_diff_imm~log_spe_repro_effort * min_temp + log_spe_repro_effort * precipitation +
               body_mass_imm + min_temp * body_mass_end_mating + body_mass_end_mating*precipitation +
               maternal_effort * precipitation + maternal_effort * min_temp + dimorphisme_imm,
               data = comb_model_2, lambda = "ML")
summary(model.avg(dredge(fit2_1, rank = "AICc", m.lim = c(NA, 4)), delta < 6))

fit2_2<-pgls(sex_diff_imm~zlog_spe_repro_effort + zbody_mass_end_mating + zprecipitation +
             zmaternal_effort, data = comb_model_2, lambda = "ML")
summary(fit2_2)
...
# The points on the Q-Q graph don't follow a a straight diagonal line.


# We found an outlier, Tachyglossus aculeatus, a species for which
# gestation and lactation for females was an extremely long period
# (i.e. 167 day). we test a new model (model 3) without this specie

#
# Model 3
#
fit3_1 <- pgls(sex_diff_imm~log_spe_repro_effort * min_temp + log_spe_repro_effort * precipitation +
               body_mass_imm + min_temp * body_mass_end_mating + body_mass_end_mating * precipitation +
               maternal_effort * precipitation + maternal_effort * min_temp + dimorphisme_imm,
               data = comb_model_3, lambda = "ML")
summary(model.avg(dredge(fit3_1, rank = "AICc", m.lim = c(NA, 4)), delta < 6))

fit3_2 <- pgls(sex_diff_imm~zlog_spe_repro_effort + zbody_mass_end_mating + zprecipitation +
               zdimorphisme_imm,data = comb_model_3, lambda = "ML")
summary(fit3_2)
...
# Equation for regression line of model 6 with mean value of late
# mating

fit3_3<-pgls(sex_diff_imm~log_spe_repro_effort + body_mass_end_mating + precipitation +
             dimorphisme_imm,data=comb_model_3, lambda = "ML")
summary(fit3_3)

eq2 = function(x){
          -180.103912 + (57.962616*2.25) + (0.049655*583.5) + (23.754942*1.2) + (0.709451*x)
      }
ggplot(model_3, aes(x = body_mass_end_mating, y = sex_diff_imm, colour = precipitation)) +
    geom_point(size = 4) + theme_classic() +
    scale_colour_gradient("Precipitation", low = "#56B1F7", high = "#132B43") +
...
eq3 = function(x){
    -180.103912 + (0.709451*-5.22) + (0.049655*583.5) + (23.754942*1.2) + (57.962616*x)
}
